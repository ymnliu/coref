...
public class CorefFunctions {
	...
	public static void fillMentionList(HashMap<String, Mention> idMentionM,List<Pair> mentionSpanList,HashMap<Pair, String> bytespanIdM){
		...
		while(menIter.hasNext()){
			...
			// Pair bytespan = new
			// Pair(mention.getExtentSt(),mention.getExtentEd());
			int startInd = 0;
			int endInd = 0;
			String phrase = "";
			if (headonly) {
				...
				// Jun 14 2015 - Sijia
				// Handle exception of "NOTE: Original string below contained double hyphens, which can confuse";
				// TODO: Though rare, some nodes have two "NOTE"s. 
				try {
					phrase = mention.getExtentCoveredText().substring(startInd, endInd).toLowerCase();
				} catch (Exception e) {
					System.out.println("Exception: " + mention.getExtentCoveredText() + ": " + startInd + " to " + endInd);
					e.printStackTrace();
					continue;
				}
			}
			...
			for(Mention mention:menList){
				...
				if(headonly){
					...
					try {
						...
						System.out.println(mention.getExtentCoveredText() + ": " + startInd + " to " + endInd);
						e.printStackTrace();
						continue;
					}
				}
	...
	public static ArrayList<CorefClusters> getKeyData(List<String> bsTokenList,HashMap<Pair,String> bytespanIdM,HashMap<String, Mention> idMentionM,HashMap<String,List<Pair>> idMentionListM, List<Pair> mentionSpanList){
		...
		for(Pair bytespan: sortedMentionSpanList){
			...
			// Thrid person pronuon filter.
			// removed for generic usage.
